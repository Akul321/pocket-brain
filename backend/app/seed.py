...
def seed_demo_data(db, user_id: int):
    today = date.today()
    ...
    for desc, amount, txn_type, category, notes, txn_date in transactions:
        ...
        db.add(Transaction(
            user_id=user_id, date=txn_date, description=desc,
            amount=amount, type=txn_type, category=category,
            notes=notes, ai_note=ai_note,
        ))

    month_str = today.strftime("%Y-%m")
    for category, limit in [
        ("Food", 8000), ("Transport", 2000), ("Shopping", 5000),
        ("Subscriptions", 1500), ("Entertainment", 2000), ("Health", 3000),
        ("Education", 5000), ("Investments", 6000), ("Miscellaneous", 3000),
    ]:
        db.add(Budget(user_id=user_id, category=category, monthly_limit=limit, month=month_str))

    for g in [
        Goal(user_id=user_id, name="Emergency Fund", target_amount=150000, current_amount=45000, monthly_contribution=5000, priority="high"),
        Goal(user_id=user_id, name="New Laptop", target_amount=80000, current_amount=22000, monthly_contribution=8000,
             deadline=date(today.year, today.month + 7 if today.month <= 5 else today.month - 5, 1), priority="medium"),
        Goal(user_id=user_id, name="Vacation Trip", target_amount=40000, current_amount=12000, monthly_contribution=4000, priority="low"),
        Goal(user_id=user_id, name="Investment Starter", target_amount=100000, current_amount=30000, monthly_contribution=5000, priority="high"),
    ]:
        db.add(g)
